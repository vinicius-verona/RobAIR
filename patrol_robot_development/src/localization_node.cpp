...
#define ticks_until_update \
    25  // 2.5 seconds static until update (when odom says we have moved since
        // last update)
...
public:
            ...
            for (float loop_y = min_y; loop_y <= max_y; loop_y += distance_resolution) {
                for (float o = min_orientation; o <= max_orientation; o += angle_resolution) {
                    if (cell_value(loop_x, loop_y) == 0)  // robair can only be at a free cell
                    {
                        ...
                        // o * 180 / M_PI, score_current);
                        // populateMarkerTopic();
                        // ROS_INFO("press enter to continue");
                        // getchar();

                        // we store the maximum score over all the possible
                        // positions in estimated_position
    ...
    int sensor_model(float x, float y, float o) {
        ...
        for (int loop = 0; loop < nb_beams; loop++, beam_angle += angle_inc) {
            // for each hit of the laser, we compute its position in the map and
