...
public:
    ...
    void update() {
        if (init_odom && init_obstacle) {  // wait for the initialization of
                                           // odometer and detect_obstacle_node
            ...
            /*     if ( only_rotation && cond_rotation )
                 {
                     ROS_INFO("processing the /rotation_to_do received at %f",
               rotation_to_do*180/M_PI); compute_rotation(); move_robot();
                 }*/
    ...
    void compute_translation() {
        ...
        ROS_INFO("translation_to_do: %f, translation_done: %f, "
                 "error_translation: %f",
        ...
        if (cond_obstacle && cond_translation) {
            ...
            // ROS_INFO("error_integral_translation: %f",
...
int main(int argc, char** argv) {
    ...
    /*if ( ros::param::get("/action_node/obstacle_detection",
    obstacle_detection) ) if ( obstacle_detection )
            ROS_INFO("obstacle_detection: true");
        else
            ROS_INFO("obstacle_detection: false");

    if ( ros::param::get("/action_node/translation_speed_max",
    translation_speed_max) ) ROS_INFO("translation_speed_max: %f",
    translation_speed_max);

    if ( ros::param::get("/action_node/translation_acceleration",
    translation_acceleration) ) ROS_INFO("translation_acceleration: %f",
    translation_acceleration);

    if ( ros::param::get("/action_node/translation_deceleration",
    translation_deceleration) ) ROS_INFO("translation_deceleration: %f",
    translation_deceleration);

    if ( ros::param::get("/action_node/rotation_acceleration",
    rotation_acceleration) ) ROS_INFO("rotation_acceleration: %f",
    rotation_acceleration);

    if ( ros::param::get("/action_node/rotation_deceleration",
