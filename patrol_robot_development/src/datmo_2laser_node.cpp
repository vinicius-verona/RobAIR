...
public:
    person_detector_tracker() {
        pub_person_detector_tracker_marker = n.advertise<visualization_msgs::Marker>(
            "one_person_detector_tracker",
        ...
        // pub_stamped_person_tracker = n.advertise<geometry_msgs::PointStamped>("goal_to_reach", 1);     // Preparing a
        // topic to publish the goal to reach.
        //    pub_person_tracker = n.advertise<geometry_msgs::Point>("goal_to_reach", 1);     // Preparing a topic to
        //    publish the goal to reach.
        pub_person_tracker =
            n.advertise<geometry_msgs::Point>("person_position", 1);  // Preparing a topic to publish the goal to reach.
    ...
    void search_local_minimum(int laser) {
        ...
        for (int loop = 0; loop < nb_cluster[laser]; loop++) {
            if (loop == 0)
                local_minimum[loop][laser] =
                    (distancePoints(origin, current_scan[cluster_end[loop][laser]][laser]) <
                     distancePoints(origin, current_scan[cluster_start[loop + 1][laser]][laser]));
            else if (loop == nb_cluster[laser] - 1)
                local_minimum[loop][laser] =
                    (distancePoints(origin, current_scan[cluster_start[loop][laser]][laser]) <
                     distancePoints(origin, current_scan[cluster_end[loop - 1][laser]][laser]));
            else
                local_minimum[loop][laser] =
                    (distancePoints(origin, current_scan[cluster_end[loop][laser]][laser]) <
                     distancePoints(origin, current_scan[cluster_start[loop + 1][laser]][laser])) &&
    ...
    void detect_legs() {
        ...
        for (int loop = 0; loop < nb_cluster[0]; loop++)
            if ((cluster_size[loop][0] < leg_size_max) && (cluster_size[loop][0] > leg_size_min)) {
                ...
                leg_dynamic[nb_legs_detected] =
                    (cluster_dynamic[loop][0] > dynamic_threshold);  // && ( cluster_size[loop][0]>leg_size_min );
                float dist_max = 0;

                if (leg_dynamic[nb_legs_detected])
                    ROS_INFO("moving leg found: %i -> cluster = %i, (%f, %f), size: %f, dynamic: %f, local_minimum: %i",
                             nb_legs_detected, loop, leg_detected[nb_legs_detected].x, leg_detected[nb_legs_detected].y,
                             cluster_size[loop][0], cluster_dynamic[loop][0], local_minimum[loop][0]);
                else
                    ROS_INFO("static leg found: %i -> cluster = %i, (%f, %f), size: %f, dynamic: %f, local_minimum: %i",
                             nb_legs_detected, loop, leg_detected[nb_legs_detected].x, leg_detected[nb_legs_detected].y,
                             cluster_size[loop][0], cluster_dynamic[loop][0], local_minimum[loop][0]);
                ...
                if (dist_max > cluster_threshold / 2) {
                    ...
                    if ((size1 < leg_size_max) && (size1 > leg_size_min)) {
                        leg_detected[nb_legs_detected].x =
                            (current_scan[cluster_start[loop][0]][0].x + current_scan[middle][0].x) / 2;
                        leg_detected[nb_legs_detected].y =
                            (current_scan[cluster_start[loop][0]][0].y + current_scan[middle][0].y) / 2;

                        ROS_INFO("leg found: %i -> cluster = %i, (%f, %f), size: %f", nb_legs_detected, loop,
                                 leg_detected[nb_legs_detected].x, leg_detected[nb_legs_detected].y, size1);
                        nb_legs_detected++;
                    }
                    if ((size2 < leg_size_max) && (size2 > leg_size_min)) {
                        leg_detected[nb_legs_detected].x =
                            (current_scan[cluster_end[loop][0]][0].x + current_scan[middle + 1][0].x) / 2;
                        leg_detected[nb_legs_detected].y =
                            (current_scan[cluster_end[loop][0]][0].y + current_scan[middle + 1][0].y) / 2;

                        ROS_INFO("leg found: %i -> cluster = %i, (%f, %f), size: %f", nb_legs_detected, loop,
                                 leg_detected[nb_legs_detected].x, leg_detected[nb_legs_detected].y, size2);
                        nb_legs_detected++;
                    }
                }
            }
    ...
    void detect_chests() {
        ...
        for (int loop = 0; loop < nb_cluster[1]; loop++)
            if ((cluster_size[loop][1] < chest_size_max) &&
                (cluster_size[loop][1] > chest_size_min) /*|| person_tracked*/) {
                ...
                // ROS_INFO("person detected2: %i -> cluster2: %i, center: (%f, %f), lcenter: (%f, %f), rcenter: (%f,
                // %f), size: %f", nb_chests_detected2, loop, cluster_middle2[loop].x, cluster_middle2[loop].y,
                // cluster_leg1[loop].x, cluster_leg1[loop].y,cluster_leg2[loop].x, cluster_leg2[loop].y,
                // cluster_size2[loop]);
                if (!chest_dynamic[nb_chests_detected])
                    ROS_INFO("static chest detected: %i -> cluster2: %i, center: (%f, %f), size: %f, dynamic: %f, "
                             "local_minimum: %i",
                             nb_chests_detected, loop, chest_detected[nb_chests_detected].x,
                             chest_detected[nb_chests_detected].y, cluster_size[loop][1], cluster_dynamic[loop][1],
                             local_minimum[loop][1]);
                else
                    ROS_INFO("moving chest detected: %i -> cluster2: %i, center: (%f, %f), size: %f, dynamic: %f, "
                             "local_minimum: %i",
                             nb_chests_detected, loop, chest_detected[nb_chests_detected].x,
                             chest_detected[nb_chests_detected].y, cluster_size[loop][1], cluster_dynamic[loop][1],
                             local_minimum[loop][1]);
            ...
            if (leg_dynamic[loop_left_leg])
                for (int loop_right_leg = loop_left_leg + 1; loop_right_leg < nb_legs_detected; loop_right_leg++)
                    if (leg_dynamic[loop_right_leg]) {
                        float two_legs_distance =
                            distancePoints(leg_detected[loop_left_leg], leg_detected[loop_right_leg]);
                        ...
                        if (person_detected)
                            for (int loop_chest = 0; loop_chest < nb_chests_detected; loop_chest++)
                                if (chest_dynamic[loop_chest]) {
                                    float left_chest_distance =
                                        distancePoints(leg_detected[loop_left_leg], chest_detected[loop_chest]);
                                    float right_chest_distance =
                                        distancePoints(leg_detected[loop_right_leg], chest_detected[loop_chest]);

                                    person_detected = (left_chest_distance < distance_level) &&
                                                      (right_chest_distance < distance_level);
    ...
    void predict_person() {
        ...
        ROS_INFO("previous_odom(%f, %f) -> current_odom(%f, %f) = t(%f, %f)", previous_odom.x, previous_odom.y,
                 current_odom.x, current_odom.y, diff_x, diff_y);
        if (diff_x || diff_y) {
        ...
        nb_pts = 0;
        ROS_INFO("previous_orientation(%f) -> current_orientation(%f) = rotation(%f)", previous_odom.z * 180 / M_PI,
                 current_odom.z * 180 / M_PI, diff_orientation * 180 / M_PI);
        if (diff_orientation) {
            for (int loop_leg_tracked = 0; loop_leg_tracked < nb_legs_tracked; loop_leg_tracked++) {
                geometry_msgs::Point current_leg = leg_tracked[loop_leg_tracked];
                leg_tracked[loop_leg_tracked].x =
                    current_leg.x * cos(diff_orientation) + current_leg.y * sin(diff_orientation);
                leg_tracked[loop_leg_tracked].y =
                    -current_leg.x * sin(diff_orientation) + current_leg.y * cos(diff_orientation);
            ...
            for (int loop_chest_tracked = 0; loop_chest_tracked < nb_chests_tracked; loop_chest_tracked++) {
                ...
                chest_tracked[loop_chest_tracked].x =
                    current_chest.x * cos(diff_orientation) + current_chest.y * sin(diff_orientation);
                chest_tracked[loop_chest_tracked].y =
                    -current_chest.x * sin(diff_orientation) + current_chest.y * cos(diff_orientation);
    ...
    void associate_person() {
        ...
        float best_uncertainty =
            (nb_legs_tracked + 1) * uncertainty_max_leg + nb_chests_tracked * uncertainty_max_chest;
                    ...
                    if (right_associated)
                        for (int loop_chest = unassociated; loop_chest < nb_chests_detected; loop_chest++) {
                            ...
                            if (chest_associated) {
                                ...
                                if (loop_left == unassociated)
                                    ROS_INFO("leg[left]:(%f, %f) is unassociated", leg_tracked[left].x,
                                             leg_tracked[left].y);
                                else
                                    ROS_INFO("leg[left]:(%f, %f) is associated with detection[%i]: (%f, %f) = %f",
                                             leg_tracked[left].x, leg_tracked[left].y, loop_left,
                                             leg_detected[loop_left].x, leg_detected[loop_left].y, left_distance);

                                if (loop_right == unassociated)
                                    ROS_INFO("leg[right]:(%f, %f) is unassociated", leg_tracked[right].x,
                                             leg_tracked[right].y);
                                else
                                    ROS_INFO("leg[right]:(%f, %f) is associated with detection[%i]: (%f, %f) = %f",
                                             leg_tracked[right].x, leg_tracked[right].y, loop_right,
                                             leg_detected[loop_right].x, leg_detected[loop_right].y, right_distance);

                                if (loop_chest == unassociated)
                                    ROS_INFO("chest[chest]:(%f, %f) is unassociated", chest_tracked[left].x,
                                             chest_tracked[left].y);
                                else
                                    ROS_INFO("chest[chest]:(%f, %f) is associated with detection[%i]: (%f, %f) = %f",
                                             chest_tracked[left].x, chest_tracked[left].y, loop_chest,
                                             chest_detected[loop_chest].x, chest_detected[loop_chest].y,
                                             chest_distance);
                                ...
                                bool check_two_legs_distance =
                                    (two_legs_distance <
                                     legs_distance_max) /*&& ( two_legs_distance > legs_distance_min )*/;
                                ...
                                if (nb_association < 2)
                                    association_ok = true;
                                else if (nb_association == 2) {
                                    if ((loop_left != unassociated) && (loop_right != unassociated) &&
                                        (check_two_legs_distance))
                                        association_ok = true;
                                    else if ((loop_left != unassociated) && (loop_chest != unassociated) &&
                                             (check_left_distance))
                                        association_ok = true;
                                    else if ((loop_right != unassociated) && (loop_chest != unassociated) &&
                                             (check_right_distance))
                                        association_ok = true;
                                } else
                                    association_ok =
                                        (check_two_legs_distance) && (check_left_distance) && (check_right_distance);
                                ...
                                if (association_ok) {
                                    ...
                                    for (int loop_leg_tracked = 2; loop_leg_tracked < nb_legs_tracked;
                                         loop_leg_tracked++)
                                    ...
                                    for (int loop_observation = 0; loop_observation < nb_legs_detected;
                                         loop_observation++)
                                    ...
                                    for (int loop_chest_tracked = 1; loop_chest_tracked < nb_chests_tracked;
                                         loop_chest_tracked++)
                                    ...
                                    for (int loop_observation = 0; loop_observation < nb_chests_detected;
                                         loop_observation++)
                                    ...
                                    while (cond) {
                                        ...
                                        for (int loop_leg_tracked = 2; loop_leg_tracked < nb_legs_tracked;
                                            ...
                                            if (leg_track_association[loop_leg_tracked] == unassociated)
                                                for (int loop_detection = 0; loop_detection < nb_legs_detected;
                                                     loop_detection++)
                                                    if (!leg_detection_association[loop_detection]) {
                                                        float current_distance =
                                                            distancePoints(leg_tracked[loop_leg_tracked],
                                                                           leg_detected[loop_detection]);
                                        ...
                                        if (cond_leg) {
                                            ROS_INFO(
                                                "leg[%i]: (%f, %f) is associated with detection[%i]: (%f, %f) = %f",
                                                min_leg_track, leg_tracked[min_leg_track].x,
                                                leg_tracked[min_leg_track].y, min_leg_detection,
                                                leg_detected[min_leg_detection].x, leg_detected[min_leg_detection].y,
                                                min_leg_distance);
                                        ...
                                        for (int loop_chest_tracked = 1; loop_chest_tracked < nb_chests_tracked;
                                            ...
                                            if (chest_track_association[loop_chest_tracked] == unassociated)
                                                for (int loop_detection = 0; loop_detection < nb_chests_detected;
                                                     loop_detection++)
                                                    if (!chest_detection_association[loop_detection]) {
                                                        float current_distance =
                                                            distancePoints(chest_tracked[loop_chest_tracked],
                                                                           chest_detected[loop_detection]);
                                        ...
                                        if (cond_chest) {
                                            ROS_INFO(
                                                "chest[%i]: (%f, %f) is associated with detection[%i]: (%f, %f) = %f",
                                                min_chest_track, chest_tracked[min_chest_track].x,
                                                chest_tracked[min_chest_track].y, min_chest_detection,
                                                chest_detected[min_chest_detection].x,
                                                chest_detected[min_chest_detection].y, min_chest_distance);
                                    ...
                                    for (int loop_leg_tracked = 2; loop_leg_tracked < nb_legs_tracked;
                                        ...
                                        if (leg_track_association[loop_leg_tracked] == unassociated) {
                                            ...
                                            ROS_INFO("leg[%i]:(%f, %f) has been unassociated", loop_leg_tracked,
                                                     leg_tracked[loop_leg_tracked].x, leg_tracked[loop_leg_tracked].y);
                                        }

                                    for (int loop_chest_tracked = 2; loop_chest_tracked < nb_chests_tracked;
                                         loop_chest_tracked++)
                                        if (chest_track_association[loop_chest_tracked] == unassociated) {
                                            current_uncertainty += chest_uncertainty[loop_chest_tracked];
                                            ROS_INFO("chest[%i]:(%f, %f) has been unassociated", loop_chest_tracked,
                                                     chest_tracked[loop_chest_tracked].x,
                                                     chest_tracked[loop_chest_tracked].y);
                                        }
                                    ...
                                    if (current_uncertainty < best_uncertainty) {
                                        ...
                                        for (int loop_leg_tracked = 0; loop_leg_tracked < nb_legs_tracked;
                                             loop_leg_tracked++)
                                            best_leg_tracked[loop_leg_tracked] =
                                                leg_track_association[loop_leg_tracked];
                                        for (int loop_detection = 0; loop_detection < nb_legs_detected;
                                             loop_detection++)
                                            best_leg_detected[loop_detection] =
                                                leg_detection_association[loop_detection];
                                        for (int loop_chest_tracked = 0; loop_chest_tracked < nb_chests_tracked;
                                             loop_chest_tracked++)
                                            best_chest_tracked[loop_chest_tracked] =
                                                chest_track_association[loop_chest_tracked];
                                        for (int loop_detection = 0; loop_detection < nb_chests_detected;
                                             loop_detection++)
                                            best_chest_detected[loop_detection] =
                                                chest_detection_association[loop_detection];
                                    }
                                } else
    ...
    void manage_tracks_legs() {
        ...
        for (int loop_leg_tracked = 0; loop_leg_tracked < nb_legs_tracked; loop_leg_tracked++)
            if (best_leg_tracked[loop_leg_tracked] != unassociated) {
                ROS_INFO("leg[%i]:(%f, %f) has been associated with detection[%i]: (%f, %f), frequency = %i",
                         loop_leg_tracked, leg_tracked[loop_leg_tracked].x, leg_tracked[loop_leg_tracked].y,
                         best_leg_tracked[loop_leg_tracked], leg_detected[best_leg_tracked[loop_leg_tracked]].x,
                         leg_detected[best_leg_tracked[loop_leg_tracked]].y, leg_frequency[loop_leg_tracked]);
                ...
                ROS_INFO("leg[%i]:(%f, %f) has been unassociated, frequency = %i", loop_leg_tracked,
                         leg_tracked[loop_leg_tracked].x, leg_tracked[loop_leg_tracked].y,
                         leg_frequency[loop_leg_tracked]);
        ...
        for (int loop_leg_tracked = 2; loop_leg_tracked < nb_legs_tracked; loop_leg_tracked++)
            if ((leg_frequency[loop_leg_tracked]) &&
                ...
                ROS_INFO("leg[%i]: (%f, %f) is suppressed", loop_leg_tracked, leg_tracked[loop_leg_tracked].x,
                         leg_tracked[loop_leg_tracked].y);
        ...
        for (int loop_leg_detected = 0; loop_leg_detected < nb_legs_detected; loop_leg_detected++)
            if (!best_leg_detected[loop_leg_detected] &&
                ((distancePoints(leg_tracked[left], leg_detected[loop_leg_detected]) < 2 * uncertainty_max_leg) ||
                 (distancePoints(leg_tracked[right], leg_detected[loop_leg_detected]) < 2 * uncertainty_max_leg))) {
                ...
                ROS_INFO("leg[%i]:(%f, %f) has been created", nb_legs_tracked, leg_detected[loop_leg_detected].x,
                         leg_detected[loop_leg_detected].y);
                nb_legs_tracked++;
            }

        // check track too close from left or right leg
        for (int loop_leg_tracked = 2; loop_leg_tracked < nb_legs_tracked; loop_leg_tracked++) {
            if ((distancePoints(leg_tracked[loop_leg_tracked], leg_tracked[left]) < cluster_threshold) &&
                (leg_frequency[left] == 0))
                ROS_WARN("track[%i] is too close of track[left]", loop_leg_tracked);
            if ((distancePoints(leg_tracked[loop_leg_tracked], leg_tracked[right]) < cluster_threshold) &&
                (leg_frequency[right] == 0))
        ...
        for (int loop_leg_tracked = 2; loop_leg_tracked < nb_legs_tracked; loop_leg_tracked++) {
            ...
            float range = ( leg_tracked[loop_leg_tracked].x - robot_position.x ) * ( leg_tracked[loop_leg_tracked].x -
            robot_position.x ) + ( leg_tracked[loop_leg_tracked].y - robot_position.y ) * (
            leg_tracked[loop_leg_tracked].y - robot_position.y ); range = sqrt(range); float angle = acos ( (
            leg_tracked[loop_leg_tracked].x - robot_position.x ) / range ); if ( leg_tracked[loop_leg_tracked].y -
            robot_position.y < 0 ) angle *= -1; angle -= robot_orientation;
    ...
    void manage_tracks_chests() {
        ...
        for (int loop_chest_tracked = 0; loop_chest_tracked < nb_chests_tracked; loop_chest_tracked++)
            if (best_chest_tracked[loop_chest_tracked] != unassociated) {
                ROS_INFO("chest[%i]:(%f, %f) has been associated with detection[%i]: (%f, %f), frequency = %i",
                         loop_chest_tracked, chest_tracked[loop_chest_tracked].x, chest_tracked[loop_chest_tracked].y,
                         best_chest_tracked[loop_chest_tracked],
                         chest_detected[best_chest_tracked[loop_chest_tracked]].x,
                         chest_detected[best_chest_tracked[loop_chest_tracked]].y, chest_frequency[loop_chest_tracked]);
                ...
                ROS_INFO("chest[%i]:(%f, %f) has been unassociated, frequency = %i", loop_chest_tracked,
                         chest_tracked[loop_chest_tracked].x, chest_tracked[loop_chest_tracked].y,
                         chest_frequency[loop_chest_tracked]);
            }
        ...
        for (int loop_chest_tracked = 1; loop_chest_tracked < nb_chests_tracked; loop_chest_tracked++)
            if ((chest_frequency[loop_chest_tracked]) &&
                (distancePoints(chest_tracked[chest], chest_tracked[loop_chest_tracked]) < 2 * uncertainty_max_chest)) {
                ...
                ROS_INFO("chest[%i]: (%f, %f) is suppressed", loop_chest_tracked, chest_tracked[loop_chest_tracked].x,
                         chest_tracked[loop_chest_tracked].y);
        ...
        // we create new track with the detection not associated
        for (int loop_chest_detected = 0; loop_chest_detected < nb_chests_detected; loop_chest_detected++)
            if (!best_chest_detected[loop_chest_detected] &&
                (distancePoints(chest_tracked[chest], chest_detected[loop_chest_detected]) <
                 2 * uncertainty_max_chest)) {
                ...
                ROS_INFO("chest[%i]:(%f, %f) has been created", nb_chests_tracked,
                         chest_detected[loop_chest_detected].x, chest_detected[loop_chest_detected].y);
                nb_chests_tracked++;
            }
        ...
        for (int loop_chest_tracked = 1; loop_chest_tracked < nb_chests_tracked; loop_chest_tracked++) {
            ...
            float range = ( chest_tracked[loop_chest_tracked].x - robot_position.x ) * (
            chest_tracked[loop_chest_tracked].x - robot_position.x ) + ( chest_tracked[loop_chest_tracked].y -
            robot_position.y ) * ( chest_tracked[loop_chest_tracked].y - robot_position.y ); range = sqrt(range); float
            angle = acos ( ( chest_tracked[loop_chest_tracked].x - robot_position.x ) / range ); if (
            chest_tracked[loop_chest_tracked].y - robot_position.y < 0 ) angle *= -1; angle -= robot_orientation;
    ...
    void estimate_person() {
        ...
        ROS_INFO("left (%f, %f), %f, %i", leg_tracked[left].x, leg_tracked[left].y, leg_uncertainty[left],
                 leg_frequency[left]);
        ROS_INFO("right (%f, %f), %f, %i", leg_tracked[right].x, leg_tracked[right].y, leg_uncertainty[right],
                 leg_frequency[right]);
        ROS_INFO("chest (%f, %f), %f, %i", chest_tracked[chest].x, chest_tracked[chest].y, chest_uncertainty[chest],
                 chest_frequency[chest]);
        // getchar();
        ...
        person_tracked =
            /*( /*( leg_frequency[left] > 0 ) && ( leg_frequency[right] > 0 ) ) ||*/ (chest_frequency[chest] > 0);
        if (!person_tracked) {
            geometry_msgs::Point position_tracked;
    ...
    void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan) {
        ...
        for (int loop = 0; loop < nb_beams; loop++, beam_angle += angle_inc) {
            ...
            // ROS_INFO("laser[%i]: (%f, %f) -> (%f, %f)", loop, range[loop], beam_angle*180/M_PI, current_scan[loop].x,
            // current_scan[loop].y);
    ...
    void scanCallback2(const sensor_msgs::LaserScan::ConstPtr& scan) {
        ...
        for (int loop = 0; loop < nb_beams; loop++, beam_angle += angle_inc) {
            ...
            current_scan[loop][1].x =
                transform_laser.x + robot_position.x + range[loop][1] * cos(beam_angle + robot_orientation);
