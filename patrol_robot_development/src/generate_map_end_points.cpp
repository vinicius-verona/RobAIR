...
private:
    ...
    // Used to store the vertices of the map
    geometry_msgs::Point end_points_positions[1000];
...
public:
    ...
    // Checks if the point p is in the line between a and b
    // PS: it seems different from standard line function because of the configuration fo the axis (x axis is the
    // standard y axis)
    bool isBetween(geometry_msgs::Point a, geometry_msgs::Point b, geometry_msgs::Point p) {
        // line is x - x1 = (x2 - x1) / (y2 - y1) * (y - y1)
